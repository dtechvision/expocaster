// taken from https://github.com/dylsteck/farcasterkit/blob/main/examples/starter/src/providers/FarcasterKitProvider.tsx
'use client';
...
export const FarcasterKitProvider: React.FC<FarcasterKitProviderProps> = ({
  baseURL = 'https://api.farcasterkit.com',
  children,
}) => {
  return (
    <FarcasterKitContext.Provider value={{ baseURL }}>{children}</FarcasterKitContext.Provider>
  );
};
...
export const useCast = (queryParams?: CastParams) => {
  ...
  useEffect(() => {
    const getCast = async () => {
      try {
        const response = await axios.get(`${baseURL}/casts/${queryParams?.hash}`, {
          params: params,
        });
        const responseData = response.data;
    ...
    const getReplies = async () => {
      try {
        const response = await axios.get(
          `${baseURL}/casts/replies?parent_hash=${queryParams?.hash ?? ''}`,
          { params: params }
        );
        const responseData = response.data;
...
export const useSearch = (queryParams?: SearchCastParams) => {
  ...
  useEffect(() => {
    const getSearch = async () => {
      try {
        const response = await axios.get(`${baseURL}/casts/search?query=${queryParams?.query}`, {
          params: params,
        });
        const responseData = response.data;
        // TODO: change API response to .casts
